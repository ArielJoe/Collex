...
@section('content')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        @if (isset($info))
            <div class="mt-10 flex flex-col items-center text-center text-gray-500 pb-8">
                ...
                <p class="text-sm text-gray-400">Pastikan peserta telah hadir dengan memindai tiket mereka.</p>
            </div>
        @else
            <div class="bg-white shadow overflow-hidden sm:rounded-lg">
                <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
                    <h3 class="text-lg leading-6 font-medium text-gray-900">Session Certificates</h3>
                    <p class="mt-1 max-w-2xl text-sm text-gray-500">Select PDF certificates to upload for each completed
                        session
                    </p>
                </div>
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">
                                            {{ \Carbon\Carbon::parse($reg['detail']['start_time'])->setTimezone('Asia/Jakarta')->format('M d, Y') }}
                                        </div>
                                        <div class="text-xs text-gray-500">
                                            {{ \Carbon\Carbon::parse($reg['detail']['start_time'])->setTimezone('Asia/Jakarta')->format('h:i A') }}
                                            -
                                            {{ \Carbon\Carbon::parse($reg['detail']['end_time'])->setTimezone('Asia/Jakarta')->format('h:i A') }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right">
                                        <form action="{{ route('organizer.certificates.store') }}" method="POST"
                                            ...
                                            <label
                                                ...
                                                <input id="certificate-{{ $reg['_id'] }}" name="certificate"
                                                    type="file" accept="application/pdf" class="sr-only" required>
                                            </label>
